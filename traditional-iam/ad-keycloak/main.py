"""
Active Directory authentication via Keycloak with GL-IAM.

This example demonstrates how to authenticate Active Directory users
through Keycloak's User Federation feature using the AD vendor preset
(sAMAccountName, objectGUID, person/organizationalPerson/user classes).
The GL-IAM code is identical to any Keycloak example — Keycloak handles
AD natively.
...
async def lifespan(app: FastAPI):
    ...
    Keycloak handles AD federation transparently — no AD-specific
    ...
    if is_healthy:
        ...
        print("AD users can authenticate through Keycloak's User Federation")
...
async def health():
    ...
    return {"status": "healthy", "provider": "keycloak", "federation": "ad"}
...
async def get_me(user: User = Depends(get_current_user)):
    ...
    Works for both AD-federated users and local Keycloak users.
...
async def member_area(
    ...
    AD users get roles via Keycloak role mapping.
    """
