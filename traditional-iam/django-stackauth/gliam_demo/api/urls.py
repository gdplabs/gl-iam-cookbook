...
urlpatterns = [
    ...
    path(
        "api/fbv/platform-admin/", views.platform_admin_fbv, name="platform-admin-fbv"
    ),
    ...
    path(
        "api/cbv/platform-admin/",
        views.PlatformAdminCBV.as_view(),
        name="platform-admin-cbv",
    ),
    # Pattern 3: Django REST Framework APIView
    path("api/drf/me/", views.MeAPIView.as_view(), name="me-drf"),
    path(
        "api/drf/member-area/",
        views.MemberAreaAPIView.as_view(),
        name="member-area-drf",
    ),
    path(
        "api/drf/admin-area/", views.AdminAreaAPIView.as_view(), name="admin-area-drf"
    ),
    path(
        "api/drf/platform-admin/",
        views.PlatformAdminAPIView.as_view(),
        name="platform-admin-drf",
    ),
